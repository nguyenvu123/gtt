...
if ( !class_exists('FG_Drupal_to_WordPress_Blocks', false) ) {
	...
	class FG_Drupal_to_WordPress_Blocks {
		...
		private function get_blocks_count() {
			...
			$pre_count = apply_filters('fgd2wp_pre_get_blocks_count', false);
			if ( $pre_count !== false ) {
				return $pre_count;
			}
		...
		private function get_blocks($limit) {
			...
			$pre_get_blocks = apply_filters('fgd2wp_pre_get_blocks', false);
			if ( $pre_get_blocks !== false ) {
				return $pre_get_blocks;
			}
			if ( version_compare($this->plugin->drupal_version, '6', '<') ) {
				// Drupal 5 and less
				return array();
		...
		private function import_block($block, $text_widget, $widget_settings, $id_widget) {
			...
			if ( !empty($content) ) {
				...
				if ( !$this->plugin->plugin_options['skip_media'] ) {
					...
					$post_media = $this->plugin->import_media_from_content($content, $block_date, array('ref' => 'block ID=' . $block['bid']));
