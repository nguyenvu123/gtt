...
if ( !class_exists('FG_Drupal_to_WordPress_Menus', false) ) {
	...
	class FG_Drupal_to_WordPress_Menus {
		...
		private function get_menus_count() {
			...
			$sql = apply_filters('fgd2wp_get_menus_count_sql', $sql);
		...
		private function get_all_menus() {
			...
			$sql = apply_filters('fgd2wp_get_menus_sql', $sql);
			$result = $this->plugin->drupal_query($sql);
		...
		private function get_menu_item($menu) {
			...
			if ( $menu['external'] == 1 ) {
				...
				if ( array_key_exists($node_id, $this->imported_posts) ) {
					$menu_item_object_id = is_array($this->imported_posts[$node_id]['post_id'])? $this->imported_posts[$node_id]['post_id'][0] : $this->imported_posts[$node_id]['post_id'];
			...
			// Description
			$options = $this->plugin->unserialize($menu['options']);
