	...
	class FG_Drupal_to_WordPress_CPT_Toolset implements FG_Drupal_to_WordPress_iCPT {
		...
		public function delete_data() {
			...
			foreach ( $toolset_tables as $table ) {
				...
				if ( !is_null($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table_with_prefix))) ) { // If the table exists
					$sql_queries[] = "TRUNCATE `$table_with_prefix`";
				}
			}
		...
		/**
		 * Register a builtin taxonomy on Toolset
		 *
		 * @since 3.28.0
		 * 
		...
		public function register_builtin_taxonomy($taxonomy, $singular, $plural, $description, $post_types, $hierarchical) {
			// Builtin taxonomies are not registered with Toolset
		}
		...
		private function create_field_group_taxonomy_relation($fields_group_post_id, $taxonomy) {
			if ( !empty($fields_group_post_id) ) {
				...
				if ( !in_array($taxonomy, $associated_taxonomies) && taxonomy_exists($taxonomy) ) {
				...
				$wpcf_custom_type = array(
					$post_type => array(
						...
						'supports' => array(
							...
							'author' => 1,
//							'custom-fields' => 1,
							'page-attributes' => $hierarchical? 1 : 0,
						),
						'rewrite' => array(
							'enabled' => 1,
							'custom' => 'normal',
		...
		public function set_post_relationship($post_id, $custom_field_name, $related_id, $custom_field, $relationship_slug) {
			if ( !isset($custom_field['entity_type']) || $custom_field['entity_type'] != 'user' ) {
				// Post/user relationship doesn't exist on Toolset
		 ...
		 * @param string $description Post type description
		 */
		public function register_repeating_field_group($post_type, $singular, $plural, $description) {
			$wpcf_custom_types = get_option('wpcf-custom-types', array());
			if ( !is_array($wpcf_custom_types) ) {
				$wpcf_custom_types = array();
			}
		 ...
		 * @return int Field group post ID
		 */
		public function create_repeating_field_group($fields_group_title, $fields_group_name, $subfields) {
