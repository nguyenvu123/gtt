...
SQL;
		...
		private function get_authors($limit=1000) {
			...
			if ( version_compare($this->plugin->drupal_version, '8', '<') ) {
				if ( $this->plugin->column_exists('users', 'timestamp') ) {
					// Drupal < 4.7
		...
		public function import_users() {
			...
			if ( isset($this->plugin->premium_options['only_authors']) && $this->plugin->premium_options['only_authors'] ) {
				return;
			}
			if ( $this->plugin->import_stopped() ) {
				return;
		...
		private function get_user_roles($user_id) {
			...
			$sql = apply_filters('fgd2wp_get_user_roles_sql', $sql, $user_id);
