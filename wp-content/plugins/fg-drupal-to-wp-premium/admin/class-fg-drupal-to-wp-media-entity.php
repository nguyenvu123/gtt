...
if ( !class_exists('FG_Drupal_to_WordPress_Media_Entity', false) ) {
	...
	class FG_Drupal_to_WordPress_Media_Entity {
		...
		public function get_nodes_types($sql) {
			$sql = preg_replace("/WHERE c.name LIKE 'node.type.%'/", "WHERE (c.name LIKE 'node.type.%' OR c.name LIKE 'media.type.%' OR c.name LIKE 'media_entity.bundle.%')", $sql);
			return $sql;
		}
		...
		public function build_node_type($node_type, $node_name, $data) {
			if ( preg_match('/^media(_entity)?\./', $node_name) && isset($data['id']) && isset($data['label']) ) {
				$node_type = array(
					'type' => $data['id'],
					'name' => $data['label'],
		...
		public function get_media_entities($sql, $prefix, $last_drupal_id, $limit, $content_type, $entity_type) {
			if ( ($entity_type == 'media') && version_compare($this->plugin->drupal_version, '8', '>=') ) {
				...
				$extra_joins = "INNER JOIN {$prefix}media nd ON nd.mid = n.mid AND nd.langcode = n.langcode";
				...
				$sql = "
					...
					LIMIT $limit
				";
				$sql = apply_filters('fgd2wp_get_media_entities_sql', $sql, $prefix, $last_drupal_id, $limit, $content_type, $entity_type);
			}
			return $sql;
		}
