	...
	class FG_Drupal_to_WordPress_Admin extends WP_Importer {
		...
		public function enqueue_scripts() {
			...
			wp_localize_script( $this->plugin_name, 'objectPlugin', array(
				...
				'enable_ftp' => defined('FGD2WPP_ENABLE_FTP'),
			));
		...
		public function ajax_importer() {
			...
			if ( !empty($current_user) && $current_user->has_cap('import') ) {
				...
				if ( $action == 'update_wordpress_info') {
					...
					if ( (($action == 'empty') && check_admin_referer('empty', 'fgd2wp_nonce_empty'))
					  || (($action == 'import') && filter_input(INPUT_POST, 'automatic_empty', FILTER_VALIDATE_BOOLEAN) && check_admin_referer( 'parameters_form', 'fgd2wp_nonce')) ) {
						$this->empty_log_file();
					}
...
SQL;
		...
		private function validate_form_info() {
			...
			return array(
				...
				'sqlite_file'				=> filter_input(INPUT_POST, 'sqlite_file', FILTER_SANITIZE_SPECIAL_CHARS),
				...
				'file_public_path_source'	=> filter_input(INPUT_POST, 'file_public_path_source', FILTER_SANITIZE_SPECIAL_CHARS),
				'file_public_path'			=> filter_input(INPUT_POST, 'file_public_path', FILTER_SANITIZE_SPECIAL_CHARS),
				'file_private_path_source'	=> filter_input(INPUT_POST, 'file_private_path_source', FILTER_SANITIZE_SPECIAL_CHARS),
				'file_private_path'			=> filter_input(INPUT_POST, 'file_private_path', FILTER_SANITIZE_SPECIAL_CHARS),
		...
		public function get_taxonomies_terms($taxonomy, $limit=1000, $taxonomy_name='', $taxonomy_module='') {
			...
			$sql = apply_filters('fgd2wp_get_taxonomies_terms_sql', $sql, $taxonomy, $extra_cols, $last_taxonomy_id, $limit);
		...
		public function insert_taxonomies_terms($terms, $taxonomy) {
			...
			foreach ( $terms as $term ) {
				...
				if ( !isset($this->imported_taxonomies[$term_id]) ) {
					// If the term is a duplicate, import it with a unique slug
					$args['slug'] = wp_unique_term_slug(sanitize_key($term['name']), (object)array(
		...
		protected function get_nodes($content_type, $limit=1000, $entity_type = 'node') {
			...
			if ( !$this->column_exists($table_name, 'vid') ) {
				// Drupal < 4.7
		...
		public function import_node($node, $content_type, $post_type, $entity_type='node') {
			...
			if ( !$this->plugin_options['skip_media'] ) {
				...
				if ( !$this->plugin_options['only_featured_image'] ) {
					...
					$post_media = $this->import_media_from_content($body, $post_date, array('ref' => 'node ID=' . $node['nid']));
		...
		public function get_node_slug($node, $language='') {
			...
			if ( $this->table_exists($table) ) {
				...
				$sql = "
					...
					LIMIT 1
				";
				$sql = apply_filters('fgd2wp_get_node_slug_sql', $sql, $node, $language);
		...
		private function get_data_body($node) {
			...
			if ( version_compare($this->drupal_version, '5', '<') && !$this->column_exists('node_revisions', 'body') ) {
				// Drupal < 4.7
				$sql = "
		...
		public function get_node_taxonomies_terms($node_id, $taxonomy='', $taxonomy_module='', $entity_type='node') {
			...
			$sql = apply_filters('fgd2wp_get_node_taxonomies_terms_sql', $sql, $node_id, $entity_type, $taxonomy, $extra_cols);
			$terms = $this->drupal_query($sql);
			$terms = apply_filters('fgd2wp_get_node_taxonomies_terms', $terms, $node_id, $entity_type, $taxonomy);
			return $terms;
		}
		...
		public function map_post_type($content_type) {
			...
			switch ( $content_type ) {
				...
				case 'page':
					$post_type = 'page';
					break;
				case 'author': // "author" is a reserved type in WordPress
					$post_type = 'authors';
					break;
				case 'order': // "order" is a reserved type in WordPress
					$post_type = 'orders';
					break;
				case 'image':
		...
		public function map_taxonomy($taxonomy) {
			...
			switch ( $taxonomy ) {
				...
				case 'post_type': // The taxonomy "post_type" prevents the posts to display on the backend with ACF
					...
					break;
				case 'status': // "status" is reserved
			...
			$wp_taxonomy = apply_filters('fgd2wp_map_taxonomy', $wp_taxonomy, $taxonomy);
			return $wp_taxonomy;
		}
		...
		public function get_and_process_featured_image($node) {
				...
				if ( !empty($field_image) ) {
					...
					$featured_image = array(
						...
						'attributs' => array(
							...
							'image_caption' => $this->get_image_attributes($field_image, 'title'),
						),
					);
			...
			if ( empty($featured_image) && $this->plugin_options['featured_image'] != 'none' ) {
				if ( isset($node['body_summary']) && isset($node['body_value']) ) {
					...
					if ( !empty($image_string) ) {
						$media = $this->import_media_from_content($image_string, $post_date, array('ref' => 'node ID=' . $node['nid']));
						if ( !empty($media) ) {
		...
		public function get_image_attributes($file, $field_name) {
			...
			if ( isset($file[$field_name]) && !empty($file[$field_name]) ) {
				...
				$data = $this->unserialize($file['data']);
			...
			if ( preg_match_all('#<(img|a)(.*?)(src|href)="(.*?)"(.*?)>#s', $content, $matches, PREG_SET_ORDER) > 0 ) {
				if ( is_array($matches) ) {
					foreach ($matches as $match ) {
						...
						// Image caption
						$image_caption = '';
						if (preg_match('#(title|data-caption)="(.*?)"#', $other_attributes, $title_matches) ) {
							$image_caption = $title_matches[2];
						}
		...
		public function import_media($name, $filename, $date='0000-00-00 00:00:00', $attributes=array(), $options=array()) {
			...
			$filename = str_replace('%', '~percent~', $filename); // for filenames containing %
			$filename = rawurldecode($filename); // for filenames with spaces or accents
			$filename = str_replace('&amp;', '~amp~', $filename); // for filenames containing &amp;
			...
			$filename = apply_filters('fgd2wp_import_media_filename', $filename, $name);
									...
									if ( array_key_exists('new_url', $media) ) {
										if ( (strpos($new_link, $old_filename) > 0) || (strpos($new_link, $media['url_old_filename']) > 0) ) {
											...
											if ( $link_type == 'img' ) { // images only
												...
												// Caption shortcode
												if ( (preg_match('/class=".*caption.*?"/', $link['old_link']) && preg_match('/title="(.*?)"/', $link['old_link'], $matches_caption)) || preg_match('/data-caption="(.*?)"/', $link['old_link'], $matches_caption) ) {
												...
												$new_link = apply_filters('fgd2wp_process_content_media_links_new_link', $new_link, $link['old_link']);
											}
										}
									}
								}
		...
		private function drupal_version() {
			...
			$version = apply_filters('fgd2wp_get_version', $version);
			return $version;
		}
		...
		public function get_drupal_variable($variable_name) {
			...
			if ( $this->table_exists('variable') ) {
				...
				if ( isset($result[0]) ) {
					...
					$variable = $this->unserialize($value);
				}
			}
			return $variable;
		}
		...
		public function get_drupal_config_like($query, $collection='') {
			...
			if ( $this->table_exists('config') ) {
				...
				$result = $this->drupal_query($sql);
				foreach ( $result as $row ) {
					$config[$row['name']] = $this->unserialize($row['data']);
				}
			}
			return $config;
		}
		 ...
		 * @since 3.37.2
		 * 
		 * @param string|resource $data Serialized data
		 * @return string Unserialized data
		 */
		public function unserialize($data) {
			if ( is_resource($data) ) { // PostgreSQL bytea type
