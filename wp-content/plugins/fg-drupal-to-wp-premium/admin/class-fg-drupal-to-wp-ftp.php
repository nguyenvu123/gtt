...
if ( !class_exists('FG_Drupal_to_WordPress_FTP', false) ) {
	...
	class FG_Drupal_to_WordPress_FTP extends FG_Drupal_to_WordPress_Download_FTP {
		...
		public function display_ftp_settings() {
			...
			$data['private_key_file'] = $this->plugin->ftp_options['private_key_file'];
		...
		private function validate_form_info() {
			...
			$private_key_file = filter_input(INPUT_POST, 'private_key_file', FILTER_SANITIZE_SPECIAL_CHARS);
			return array(
				...
				'private_key_file'	=> isset($private_key_file)? $private_key_file : '',
			);
		}
