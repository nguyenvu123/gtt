...
class FG_Drupal_to_WordPress_Premium {
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-fg-drupal-to-wp-cpt-pods.php';
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_filter( 'fgd2wp_map_taxonomy', $plugin_custom_content, 'map_taxonomy' );
		...
		$this->loader->add_action( 'fgd2wp_post_import', $plugin_custom_content, 'import_terms_relations', 20 );
		...
		$this->loader->add_filter( 'fgd2wp_pre_process_node', $plugin_media, 'import_media', 10, 1 );
	...
	public function acf_hack() {
		if ( function_exists('acf_update_setting') ) {
			if ( function_exists('wp_get_environment_type') && in_array(wp_get_environment_type(), array('local', 'development')) ) { // Development mode only
